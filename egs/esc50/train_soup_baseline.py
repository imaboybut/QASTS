...
def train_candidate_with_diversity(model, fold, candidate_idx, num_epochs=8):
    ...
    sample_rate = np.random.choice([16000])
    ...
    args = argparse.Namespace(
        ...
        n_print_steps=100,
        exp_dir=f'./exp/soup_fold{fold}_candidate{candidate_idx}_norate',
        wa=False,
        target_sample_rate=sample_rate  # Diversity:  sampling rate
    )
    ...
    print(f"   Candidate {candidate_idx} final accuracy: {final_acc:.4f}")
    
    #    
    with open(log_file, 'a') as f:
        f.write(f"\nFinal Accuracy: {final_acc:.4f}\n")
    
    return args.exp_dir, final_acc
...
def create_model_soup(candidate_paths, fold):
    ...
    # Soup  
    soup_dir = f'./exp/soup_fold{fold}_final_norate'
...
def train_soup_for_fold(fold, num_candidates=10, seed=42):
    ...
    for i in range(num_candidates):
        ...
        exp_dir, acc = train_candidate_with_diversity(candidate_model, fold, i, num_epochs=8)
        candidate_paths.append(exp_dir)
        candidate_accuracies.append(acc)
    ...
    print(f"{'='*80}")
    print(f"Original QAT Accuracy: {qat_accuracy:.4f}")
    print(f"Candidate Accuracies: {[f'{acc:.4f}' for acc in candidate_accuracies]}")
    print(f"Model Soup Accuracy: {soup_accuracy:.4f}")
    print(f"Improvement: {(soup_accuracy - qat_accuracy)*100:.2f}%")
    
    #    
    summary_file = f'./exp/soup_fold{fold}_final_norate/summary.txt'
