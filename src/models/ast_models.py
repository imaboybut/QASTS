...
from models.quant.quantization.modules.utils import replace_module_by_qmodule_deit, replace_module_by_qmodule_swin
...
class ASTModel(nn.Module):
    ...
    def __init__(self, label_dim=527, fstride=10, tstride=10, input_fdim=128, input_tdim=1024, imagenet_pretrain=True, audioset_pretrain=False, model_size='base384', verbose=True):
        ...
        if audioset_pretrain == False:
            if model_size == 'tiny224':
                ...
                try:
                    ...
                    try:
                        ...
                        # Fallback to base model without distillation
                        self.v = timm.create_model('deit_base_patch16_384', pretrained=imagenet_pretrain)
            else:
            ...
            # Also update the patch_embed to use custom PatchEmbed to avoid size checks
            self.v.patch_embed = PatchEmbed(img_size=(input_fdim, input_tdim), patch_size=16, in_chans=1, embed_dim=self.original_embedding_dim)
            self.v.patch_embed.proj = new_proj
            self.v.patch_embed.num_patches = num_patches
...
def get_qat_model(model, args):
    ...
    if args.model_type == 'deit':
        qat_model = replace_module_by_qmodule_deit(qat_model, qconfigs, pretrained_initialized = args.pretrained_initialized,  \
            qk_reparam=args.qk_reparam, qk_reparam_type = args.qk_reparam_type)
    elif args.model_type == 'swin':
        qat_model = replace_module_by_qmodule_swin(qat_model, qconfigs, pretrained_initialized = args.pretrained_initialized,  \
        qk_reparam=args.qk_reparam, qk_reparam_type = args.qk_reparam_type)
...
if __name__ == '__main__':
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    input_tdim = 100
    ast_mdl = ASTModel(input_tdim=input_tdim).to(device)
    print("\n--------------- ASTModel Structure ---------------")
    print(ast_mdl)
    print("----------------------------------------------------\n")
    ...
    print("------------------------  ----------------------------\n")
    print("------------------------  ----------------------------\n")
    parser = argparse.ArgumentParser(description='QAT Model Training')
    ...
    print(f"Weight Quantization Bit-width: {args.wq_bitw}")
    print(f"Activation Layer: {args.act_layer}")

    
    input_tdim = 100
    ast_mdl = ASTModel(input_tdim=input_tdim).to(device)
    print("----------------------------------------------------")
    ast_mdl = get_qat_model(ast_mdl, args)
    ...
    # input a batch of 10 spectrogram, each with 512 time frames and 128 frequency bins
    test_input = torch.rand([10, input_tdim, 128]).half().to(device)
    test_output = ast_mdl(test_input)
    # output should be in shape [10, 50], i.e., 10 samples, each with prediction of 50 classes.
    print(test_output.shape)
