...
class StatsQuantizer(nn.Module):
    def __init__(self, num_bits, clip_learnable):
        ...
        init_act_clip_val = 2 ** (self.num_bits - 1)
    ...
    def forward(self, weight):
        ...
        if not self.initialized:
            if len(weight.shape) == 2:
                scaling_factor = torch.mean(abs(real_weights),dim=1,keepdim=True) / (2 ** (self.num_bits - 1) - 1)
            elif len(weight.shape) == 3:
                scaling_factor = torch.mean(torch.mean(abs(real_weights),dim=-1,keepdim=True),dim=0,keepdim=True) / (2 ** (self.num_bits - 1) - 1)

            self.register_buffer('scaling_factor', scaling_factor.detach())
            self.s = self.scaling_factor.squeeze()
            self.initialized = True
        else:
            scaling_factor = self.scaling_factor

        scaled_weights = real_weights/scaling_factor
        cliped_weights = torch.clamp(scaled_weights, min=-(2 ** (self.num_bits - 1)), max=(2 ** (self.num_bits - 1) - 1))
        quan_weights_no_grad = scaling_factor * torch.round(cliped_weights)
...
class StatsQuantizer_specific_4_qkreparam_cga(nn.Module):
    ...
    def forward(self, weight):
        ...
        if self.training:
            not_freeze_idx = torch.zeros_like(real_weights)
