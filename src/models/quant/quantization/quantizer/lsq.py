...
class LsqQuantizerWeight(torch.nn.Module):
    ...
    def init_from(self, x, *args, **kwargs):
        if self.per_channel:
            ...
            if self.learnable:
                self.s = torch.nn.Parameter(torch.zeros(x.shape[-2],device=x.device))
                self.s.data.copy_(init_val.to(x.device))
            else:
                self.s = torch.nn.Parameter(torch.zeros(x.shape[-2],device=x.device),requires_grad=False)
                self.s.data.copy_(init_val.to(x.device))
        else:
            init_val = x.detach().abs().mean() * 2 / (self.thd_pos ** 0.5)
            if self.learnable: 
                self.s = torch.nn.Parameter(torch.zeros(1,device=x.device))
                self.s.data.copy_(init_val.to(x.device))
            else: 
                self.s = torch.nn.Parameter(torch.zeros(1,device=x.device),requires_grad=False)
                self.s.data.copy_(init_val.to(x.device))
...
class LsqQuantizer4img(torch.nn.Module):
    ...
    def init_from(self, x, *args, **kwargs):
        if self.per_channel:
            ...
            if self.learnable:
                self.s = torch.nn.Parameter(torch.zeros(x.shape[1],device=x.device))
                self.s.data.copy_(init_val.to(x.device))
            else:
                self.s = torch.nn.Parameter(torch.zeros(x.shape[1],device=x.device),requires_grad=False)
                self.s.data.copy_(init_val.to(x.device))
...
class LsqQuantizer4Conv2d(torch.nn.Module):
    ...
    def init_from(self, x, *args, **kwargs):
        if self.per_channel:
            ...
            if self.learnable:
                self.s = torch.nn.Parameter(torch.zeros(x.shape[0],device=x.device))
                self.s.data.copy_(init_val.to(x.device))
            else:
                self.s = torch.nn.Parameter(torch.zeros(x.shape[0],device=x.device),requires_grad=False)
                self.s.data.copy_(init_val.to(x.device))
...
class LsqQuantizer(torch.nn.Module):
    ...
    def init_from(self, x, *args, **kwargs):
        if self.per_channel:
            if len(x.shape) == 3:
                # CRITICAL FIX: For 3D [batch, seq, channels], quantize per channel (last dim), not per position
            ...
            if self.learnable:
                # Use last dimension for 3D tensors (channels), appropriate dimension for others
                scale_dim = x.shape[-1] if len(x.shape) == 3 else (x.shape[-1] if len(x.shape) == 2 else x.shape[1])
                self.s = torch.nn.Parameter(torch.zeros(scale_dim,device=x.device))
                self.s.data.copy_(init_val.to(x.device))
            else:
                scale_dim = x.shape[-1] if len(x.shape) == 3 else (x.shape[-1] if len(x.shape) == 2 else x.shape[1])
                self.s = torch.nn.Parameter(torch.zeros(scale_dim,device=x.device),requires_grad=False)
                self.s.data.copy_(init_val.to(x.device))
        else:
            init_val = x.detach().abs().mean() * 2 / (self.thd_pos ** 0.5)
            if self.learnable: 
    ...
    def forward(self, x):
        if self.per_channel:
            if (not self.initialized_alpha):
                self.init_from(x)
            # CRITICAL FIX: Correctly expand scale dimensions based on input shape
            if len(x.shape) == 3:
                # For 3D [batch, seq, channels], scale is per-channel
                alpha = self.s.unsqueeze(0).unsqueeze(0)  # [1, 1, channels]
            elif len(x.shape) == 2:
                # For 2D [batch, channels], scale is per-channel  
                alpha = self.s.unsqueeze(0)  # [1, channels]
            else:
                alpha = torch.unsqueeze(self.s,dim=-1)
        ...
        if self.per_channel:
            if len(x.shape) == 3:
                s_grad_scale = 1.0 / ((self.thd_pos * x.shape[0]*x.shape[1]) ** 0.5)  # Fixed: use first two dims
            elif len(x.shape) == 2:
                s_grad_scale = 1.0 / ((self.thd_pos * x.shape[0]) ** 0.5)  # Fixed: use batch dim
            elif len(x.shape) == 4:
                s_grad_scale = 1.0 / ((self.thd_pos * x.shape[0]*x.shape[2]* x.shape[3]) ** 0.5)  # Fixed dimensions
