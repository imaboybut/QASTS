...
from ...deit_vision_transformer import Attention as deit_attention
...
class QAttention(deit_attention):
    def __init__(self, m: deit_attention, weight_bits=8, input_bits=8, aq_learnable=True, wq_learnable = True,
        ...
        self.qkv = QLinear(
            m = m.qkv,  # FIX: Use m.qkv, not self.qkv!
        ...
        self.proj = QLinear(
            m = m.proj,  # FIX: Use m.proj, not self.proj!
            weight_bits = weight_bits,
            input_bits = input_bits,
            weight_channelwise = weight_channelwise,
            input_channelwise = input_channelwise,
        ...
        # CRITICAL FIX: Use per-tensor quantization for Q and K (not per-channel)
        # Q and K have shape [B, num_heads, N, C//num_heads] - per-channel doesn't work well here
        self.quan_a_q_fn = LsqQuantizer(bit=input_bits,all_positive=False,per_channel=False, learnable =  aq_learnable).to(m.qkv.weight.device)
        self.quan_a_k_fn = LsqQuantizer(bit=input_bits,all_positive=False,per_channel=False, learnable =  aq_learnable).to(m.qkv.weight.device)
        # Use per-tensor for V as well for consistency
        self.quan_a_v_fn = LsqQuantizer4v(bit=input_bits,all_positive=False,per_channel=False, learnable =  aq_learnable).to(m.qkv.weight.device)

        self.move_qkv_b4 = LearnableBias(m.qkv.in_features*3, device=m.qkv.weight.device)
        self.move_q_aft = LearnableBias(m.qkv.in_features, device=m.qkv.weight.device)
        self.move_k_aft = LearnableBias(m.qkv.in_features, device=m.qkv.weight.device)
        self.move_v_aft = LearnableBias(m.qkv.in_features, device=m.qkv.weight.device)

        # Use per-tensor for softmax - attention weights should have consistent quantization
        self.quan_a_softmax_fn = LsqQuantizer(bit=input_bits,all_positive=True,per_channel=False, learnable =  aq_learnable).to(m.qkv.weight.device)
    ...
    def forward(self, x):
        ...
        # CRITICAL FIX: Do NOT quantize softmax output!
        # Softmax probabilities need fine granularity, quantizing them destroys attention
...
class QAttention_qkreparam(deit_attention):
    def __init__(self, m: deit_attention, weight_bits=8, input_bits=8, aq_learnable=True, wq_learnable = True,
        ...
        self.quant_x_4_qkv =  LSQ_input(bit = input_bits, all_positive= False, learnable= aq_learnable, learanbaleBiasdim=m.qkv.in_features,device=m.qkv.weight.device)
        ...
        self.qk_quant = StatsQuantizer(num_bits=self.weight_bits, clip_learnable=wq_learnable).to(m.qkv.weight.device) # num_heads*in_features, in_features
        self.v_quant = StatsQuantizer(num_bits=self.weight_bits, clip_learnable=wq_learnable).to(m.qkv.weight.device)
        ...
        ## THIS IS FOR QK PART
        self.quan_a_qkx_fn = LsqQuantizer(bit=input_bits,all_positive=False,per_channel=True, learnable =  aq_learnable).to(m.qkv.weight.device) # for W_q*W_k*X -> B, num_heads, N, C//numheads
        # Use per-tensor for V as well for consistency
        self.quan_a_v_fn = LsqQuantizer4v(bit=input_bits,all_positive=False,per_channel=False, learnable =  aq_learnable).to(m.qkv.weight.device)

        # for W_q*W_k*X
        self.move_qkx_b4 = LearnableBias(self.num_heads * m.qkv.in_features, device=m.qkv.weight.device)
        self.move_qkx_aft = LearnableBias(self.num_heads * m.qkv.in_features, device=m.qkv.weight.device)
        
        # for v
        self.move_v_b4 = LearnableBias(m.qkv.in_features, device=m.qkv.weight.device)
        self.move_v_aft = LearnableBias(m.qkv.in_features, device=m.qkv.weight.device)
...
class QAttention_qkreparam_4_cga(deit_attention):
    def __init__(self, m: deit_attention, clip_val=2.5, weight_bits=8, input_bits=8, aq_learnable=True, wq_learnable = True,
        ...
        self.qk_quant = StatsQuantizer_specific_4_qkreparam_cga(num_bits=self.weight_bits, clip_learnable=wq_learnable,boundaryRange=boundaryRange).to(m.qkv.weight.device) # num_heads*in_features, in_features
        self.v_quant = StatsQuantizer(num_bits=self.weight_bits, clip_learnable=wq_learnable).to(m.qkv.weight.device)#.to(m.qkv.weight.device)
...
class QAttention_lsq(deit_attention):
    def __init__(self, m: deit_attention, clip_val=2.5, weight_bits=8, input_bits=8, aq_learnable=True, wq_learnable = True,
        ...
        self.input_bits = input_bits
        self.input_channelwise = input_channelwise
        device = m.qkv.weight.device
        self.qkv = LSQ_w_and_act_QLinear(
            m = m.qkv,  # CRITICAL FIX: Use m.qkv (original with pretrained weights), not self.qkv!
            weight_bits = weight_bits,
            input_bits = input_bits,
            weight_channelwise = weight_channelwise,
            input_channelwise = input_channelwise,
        ...
        self.proj = LSQ_w_and_act_QLinear(
            m = m.proj,  # CRITICAL FIX: Use m.proj (original with pretrained weights), not self.proj!
            weight_bits = weight_bits,
            input_bits = input_bits,
            weight_channelwise = weight_channelwise,
            input_channelwise = input_channelwise,
