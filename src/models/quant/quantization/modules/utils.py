...
try:
    from timm.layers.attention import Attention as TimmLayersAttention
    from timm.layers.mlp import Mlp as TimmLayersMlp
except ImportError:
    TimmLayersAttention = None
    TimmLayersMlp = None
...
from ...swin import ShiftedWindowAttention
...
QMODULE_MAPPINGS = {
    ...
    TimmAttention: QAttention,
    TimmMlp: QMLP
}

if TimmLayersAttention is not None:
    QMODULE_MAPPINGS[TimmLayersAttention] = QAttention
if TimmLayersMlp is not None:
    QMODULE_MAPPINGS[TimmLayersMlp] = QMLP

##  0: QAttention_qkreparam, 1: QAttention_qkreparam_4_cga
QMODULE_MAPPINGS_QK_REPARAM = [
    {
        torch.nn.Linear: QLinear,
        TimmAttention: QAttention_qkreparam,
        TimmMlp: QMLP
    },
    {
        torch.nn.Linear: QLinear,
        TimmAttention: QAttention_qkreparam_4_cga,
        TimmMlp: QMLP
    }
]

if TimmLayersAttention is not None:
    for qmap in QMODULE_MAPPINGS_QK_REPARAM:
        qmap[TimmLayersAttention] = qmap[TimmAttention]
if TimmLayersMlp is not None:
    for qmap in QMODULE_MAPPINGS_QK_REPARAM:
        qmap[TimmLayersMlp] = QMLP
QMODULE_MAPPINGS_W_AND_ACT = {
    torch.nn.Linear: LSQ_w_and_act_QLinear,
    TimmAttention: QAttention_lsq,
    TimmMlp: LSQ_w_and_act_QMLP
}

if TimmLayersAttention is not None:
    QMODULE_MAPPINGS_W_AND_ACT[TimmLayersAttention] = QAttention_lsq
if TimmLayersMlp is not None:
    QMODULE_MAPPINGS_W_AND_ACT[TimmLayersMlp] = LSQ_w_and_act_QMLP
        ...
        if qconfigs[list(qconfigs.keys())[0]]["weight"]["mode"] == 'lsq' and qconfigs[list(qconfigs.keys())[0]]["act"]["mode"] == 'lsq':
            ...
            for name, cfg in qconfigs.items():
                if name.endswith("patch_embed.proj"):
                    ...
                    qmodule = LSQ_QConv2d(
                        m = module,
                        input_fdim=model.input_fdim, 
                        input_tdim=model.input_tdim,
                ...
                elif name.endswith("head") or name.endswith("head_dist") or name == "mlp_head.1":
                    module = get_module_by_name(model, name)
