...
if args.model == 'ast':
    ...
    # Dynamically set float checkpoint path based on fold
    import re
    fold_match = re.search(r'fold(\d)', args.exp_dir)
    if fold_match:
        fold_num = fold_match.group(1)
        float_ckpt_path = f"../../egs/esc50/exp/fp_fold{fold_num}_baseline/models/final_audio_model.pth"
        print(f"Loading FP model for fold {fold_num} from: {float_ckpt_path}")
    else:
        float_ckpt_path = "../../egs/esc50/exp/test-esc50-f10-t10-impTrue-aspTrue-b48-lr1e-5/fold1/models/final_audio_model.pth"
        print(f"Loading default FP model from: {float_ckpt_path}")
